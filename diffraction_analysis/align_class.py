# copied from leofang/ptycho/align_class.py
...
(default = 1)
   ...
   # Partial-pixel shift
       ...
       mlarge=m*2
       nlarge=n*2
       CC=npy.zeros([mlarge,nlarge], dtype=buf1ft.dtype) # assume buf1ft.dtype == buf2ft.dtype
...
def subpixel_align(ref, tmp, threshold_flag, threshold, subpixel_flag):
    ...
    if npy.size(ndims) == 2:
       ...
       if threshold_flag == 1:
          ref_tmp = npy.zeros((nx,ny), dtype=ref.dtype)
          index = npy.where(npy.abs(ref) >= threshold*npy.max(npy.abs(ref)))
          ref_tmp[index] = 1.
          tmp_tmp = npy.zeros((nx,ny), dype=tmp.dtype)
          index = npy.where(npy.abs(tmp) >= threshold*npy.max(npy.abs(tmp)))
          tmp_tmp[index] = 1.
       ...
       e, p, cl, r, array_shift = dftregistration(ref_fft, tmp_fft, usfac=100)
       x_shift = cl
       y_shift = r
       ...
       tmp_fft_new = tmp_fft_new.astype(ref.dtype)
...
def remove_phase_ramp(tmp, threshold_flag, threshold, subpixel_flag):
    tmp_tmp, x_shift, y_shift = subpixel_align(sf.ifftshift(sf.ifftn(sf.fftshift(npy.abs(tmp)))), 
                                               sf.ifftshift(sf.ifftn(sf.fftshift(tmp))), 
                                               threshold_flag, threshold, subpixel_flag)
